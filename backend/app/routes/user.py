...
def get_profile(current_user):
    ...
    # current_user is already fetched by middleware
    user_data = {
        ...
        "last_active": current_user.get("last_active", "Just now"),
        "sos_status": current_user.get("sos_status", "Inactive")
    }
...
def add_emergency_contact(current_user):
    ...
    # Enforce limit of 6 contacts
    current_contacts = current_user.get("emergency_contacts", [])
    if len(current_contacts) >= 6:
        return jsonify({"error": "Maximum limit of 6 emergency contacts reached"}), 400

    # Check for duplicate phone
    for c in current_contacts:
        if c.get("phone") == data.get("phone").strip():
            return jsonify({"error": "This contact number already exists"}), 400
...
def edit_emergency_contact(current_user, contact_id):
    ...
    result = mongo.users.update_one(
        {"_id": current_user["_id"], "emergency_contacts.id": contact_id},
        {"$set": {
            "emergency_contacts.$.name": data.get("name").strip(),
            "emergency_contacts.$.phone": data.get("phone").strip(),
            "emergency_contacts.$.address": data.get("address", "").strip(),
            "emergency_contacts.$.relation": data.get("relation", "").strip()
        }}
    )
